...
form.addEventListener("submit", function (x) {
  ...
  if (!depositor.value) {
    depositorError.textContent = "depositor name is required";
    isValid = false;
  } else {
    depositorError.textContent = "oh you are right";
  ...
  if (!accountname.value) {
    accountError.textContent = "account  name is required";
    isValid = false;
  } else {
...
function rate() {
  ...
  let toa = myinput1 + myinput2 + myinput3;
  document.getElementById("total").value = toa;
